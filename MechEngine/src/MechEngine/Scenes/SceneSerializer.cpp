...
namespace MechEngine {
	...
	bool SceneSerializer::Deserialze(const std::string& filepath){
		...
		if (objects)
		{
			...
			{
				uint64_t uuid = object["Object"].as<uint64_t>(); // TODO
				...
				ME_CORE_TRACE("Deserialized object with ID = {0}, name = {1}", uuid, name);

				Object deserializedObject = m_Scene->CreateObject(name);
	...
	void SceneSerializer::SerialzeRuntime(const std::string& filepath){
		ME_CORE_ASSERT(false, "RUNTIME SERIALIZATION NOT IMPLEMENTED");
	}
