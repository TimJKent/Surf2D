...
namespace MechEngine {
	...
	std::unordered_map<GLenum, std::string> OpenGLShader::PreProcess(const std::string& source) {
		...
		while (pos != std::string::npos) {
			...
			ME_CORE_ASSERT(type == "vertex" || type == "fragment" || type == "pixel", "Invalid shader type specified");
	...
	void OpenGLShader::Compile(const std::unordered_map<GLenum, std::string>& shaderSources) {
		...
		ME_CORE_ASSERT(shaderSources.size() <= 2, "Shader files must contain 2 shaders");
		...
		if (isLinked == GL_FALSE) {
			...
			ME_CORE_ERROR("{0}", infoLog.data());
			ME_CORE_ASSERT(false, "Shader Linking Failure!");
			return;
		}
